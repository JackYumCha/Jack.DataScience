...
namespace MvcAngular.Generator
{
    ...
    public class AngularGenerator
    {
        ...
        internal static void GenerateClients(string rootPathCodeEmitting, string[] assemblyNamesToScan = null)
        {
            ...
            // for validating the the angular service type is derived from object, must be a class
            var controllerBaseType = typeof(object);
        ...
        internal static FileEmitResult TranspilieAngularService(Type service, Type controllerBaseType, string rootPathCodeEmitting, string baseDirectory, string assemblyName, string AutoGenratedCodeDeclaration, Dictionary<string, bool> filesInAssembly)
        {
            ...
            foreach (var method in service.GetMethods())
            {
                ...
                if (method.GetCustomAttribute<RpcAttribute>() == null)
                    continue;
                TranspilePostMethod(service, method, stb, baseroute, imports);
            }
        ...
        internal static void TranspilePostMethod(Type service, MethodInfo method, StringBuilder stb, string baseroute, HashSet<string> imports)
        {
            ...
            {
                var tsReturnType = TypeMapping(method.ReturnType, imports, service);
                //there is only one complex value, which is posted via body as json.
                stb.AppendFormat("\tpublic {0}({1}): Observable<Response> {{", CamelCase(method.Name), ResolveParameters(method.GetParameters(), imports, service), tsReturnType); stb.AppendLf();
